...
//Asignamos el svg tablero a una variable
    ...
    if (modo == false){//no se ha seleccionado ninguna pieza
      ...
      if (piezas[num] == 0){//casilla destino vacia
        ...
        if (comprobar_movimiento(num) == true){
          ...
          //MATAR AL PASO
          if ((piezas[origen] == "PN" && num == origen-16) || (piezas[origen] == "PB" && num == origen+16)){//Susceptibles al paso
            socket.emit("paso_true");
          }else{
            socket.emit("paso_false");
          }
          if (piezas[origen] == "PB" && piezas[num] == 0 && num == origen+9){//Cazada al paso de blancas
            socket.emit("paso_ban", origen+1);
          }
          if (piezas[origen] == "PB" && piezas[num] == 0 && num == origen+7){//Cazada al paso de blancas
              socket.emit("paso_ban", origen-1);
          }
          if (piezas[origen] == "PN" && piezas[num] == 0 && num == origen-9){//Cazada al paso de negras
            socket.emit("paso_nab", origen-1);
          }
          if (piezas[origen] == "PN" && piezas[num] == 0 && num == origen-7){//Cazada al paso de negras
              socket.emit("paso_nab", origen+1);
          }

          //ASCENSOS
          if (piezas[origen] == "PB" && num > 55){
            socket.emit("ascenso_b", origen, (num%8)*60, 420, 20, 20);
          }
          if (piezas[origen] == "PN" && num < 8){
            piezas[origen] = "QN";
            socket.emit("ascenso_bçn", origen, (num)*60, 420, 20, 20);
          }

          //ENROQUES
          if (piezas[origen] == "KN" && num == origen+2){//Enroque negras largo
            socket.emit("mover", num-1, num+2);
            for(var i = 0; i < 16; i++){//movimiento visual negras
              socket.emit("movimiento_n", num-1, i, num+2);
            }
          }
          if (piezas[origen] == "KN" && num == origen-2){//Enroque negras corto
            socket.emit("mover", num+1, num-1);
            for(var i = 0; i < 16; i++){//movimiento visual negras
              socket.emit("movimiento_n", num+1, i, num-1);
            }
          }
          if (piezas[origen] == "KB" && num == origen+2){//Enroque blancas largo
            socket.emit("mover", num-1, num+2);
            for(var i = 0; i < 16; i++){//movimiento visual blancas
              socket.emit("movimiento_b", num-1, i, num+2);
            }
          }
          if (piezas[origen] == "KB" && num == origen-2){//Enroque blancas corto
            socket.emit("mover", num+1, num-1);
            for(var i = 0; i < 16; i++){//movimiento visual blancas
              socket.emit("movimiento_b", num+1, i, num-1);
            }
          }

          socket.emit("mover", num, origen);
          for(var i = 0; i < 16; i++){//movimiento visual blancas
            socket.emit("movimiento_b", num, i, origen);
          }
          for(var i = 0; i < 16; i++){//movimiento visual negras
            socket.emit("movimiento_n", num, i, origen);
          }
        ...
        piezas[num] == "TB" || piezas[num] == "QB" || piezas[num] == "KB")){
          if (comprobar_movimiento(num) == true){
            if (piezas[origen] == "PN" && num < 8){
              ...
              // if (negras[i][2] == num){
                // negras[i][0] =
                tablero.image("../../img/QN.png", (num)*60, 00, 20, 20);
              // }
            }
            if(piezas[num] == "KB"){
              fin = "negras";
        ...
        piezas[num] == "TN" || piezas[num] == "QN" || piezas[num] == "KN")){
          if (comprobar_movimiento(num) == true){
            if (piezas[origen] == "PB" && num > 55){
              ...
              // if (blancas[i][2] == num){
                // blancas[i][0] =
                tablero.image("../../img/QB.png", (num%8)*60, 420, 20, 20);
              // }
            }
            if(piezas[num] == "KN"){
            fin = "blancas";
  ...
  socket.on("prueba_mov_n", function(num, i, origen){
  ...
  socket.on("prueba_mov_b", function(num, i, origen){
  ...
  socket.on("final", function(data){
    if(data == "blancas"){
      alert("Fin de la partida, ganan las blancas");
    }else{
      alert("Fin de la partida, ganan las negras");
    }
    socket.emit("salir");
  });

  socket.on("ban_paso", function(data){
    for(var i = 0; i < 16; i++){//localizar y eliminar pieza negra del tablero
      if (negras[i][2] == data){
        socket.emit("captura_n", i);
        negras[i][2] = null;
    ...
    socket.emit("pieza_0", data);
  });

  socket.on("nab_paso", function(data){
  ...
  socket.on("true_paso", function(){
    paso_factible = true;
  })

  socket.on("false_paso", function(){
    paso_factible = false;
  })
  ...
  function movimiento_pb(tipo,num){
    var cont_mov = 0;
    if (tipo[num+9] == 0 && num >= 32 && num <= 39 && tipo[origen+1] == "PN" && paso_factible == true){
      tab[num+9].attr({
        fill: 'red'
      });
    ...
    if (tipo[num+7] == 0 && num >= 32 && num <= 39 && tipo[origen-1] == "PN" && paso_factible == true){
      tab[num+7].attr({
        fill: 'red'
      });
  ...
  function movimiento_pn(tipo,num){
    var cont_mov = 0;
    if (tipo[num-7] == 0 && num >= 24 && num <= 31 && tipo[origen+1] == "PB" && paso_factible == true){
      tab[num-7].attr({
        fill: 'red'
      });
      movimientos.push(num-7);
    ...
    if (tipo[num-9] == 0 && num >= 24 && num <= 31 && tipo[origen-1] == "PB" && paso_factible == true){
      tab[num-9].attr({
        fill: 'red'
      });
      movimientos.push(num-9);
  ...
  function movimiento_kb(tipo,num){
    var pos_cas = 0;
    if (num == 3 && tipo[num+1] == 0 && tipo[num+2] == 0 && tipo[num+3] == 0 && tipo[num+4] == "TB"){
      tab[num+2].attr({
        fill: 'red'
      });
      movimientos.push(num+2);
      pos_cas++;
    }
    if (num == 3 && tipo[num-1] == 0 && tipo[num-2] == 0 && tipo[num-3] == "TB"){
      tab[num-2].attr({
        fill: 'red'
      });
      movimientos.push(num-2);
  ...
  function movimiento_kn(tipo,num){
    var pos_cas = 0;
    if (num == 59 && tipo[num+1] == 0 && tipo[num+2] == 0 && tipo[num+3] == 0 && tipo[num+4] == "TN"){
      tab[num+2].attr({
        fill: 'red'
      });
      movimientos.push(num+2);
    ...
    if (num == 59 && tipo[num-1] == 0 && tipo[num-2] == 0 && tipo[num-3] == "TN"){
      tab[num-2].attr({
        fill: 'red'
      });
      movimientos.push(num-2);
