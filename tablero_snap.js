...
//Asignamos el svg tablero a una variable
  ...
  function identificar_cas(piezas, num, modo){
    if (modo == false){//no se ha seleccionado ninguna pieza
      ...
      if (piezas[num] != 0){//casilla destino ocupada
        ...
        }else if ((piezas[origen] == "PN" || piezas[origen] == "CN" || piezas[origen] == "AN" ||
...
//Con el 'this', seleccoionamos el objeto casilla
  ...
  function turno(){
    socket.emit('comprobar_turno');
  }

  socket.on('establecer_orden', function(orden_jug){
    turno = orden_jug;
  });

  socket.on('comp_turno', function(){
    for (var j = 0; j < 64; j++){
      if (tab[j].node == this){
        modo = identificar_cas(pieza, j, modo);
      }
  ...
  socket.on('comp_noturno', function(){
